...
def rand_sums(
    ...
    **kwargs,
    ...
    q_engine = nminer.sql.query.QueryEngine(connection, table, cmp_pred, cache)
    s_gen = nminer.text.sum.SumGenerator(
        all_preds, preamble, dim_cols, dims_tmp, agg_cols, aggs_txt, q_engine
    )
        ...
        if rand_facts is not None:
            ...
            with torch.no_grad():
                ...
                mnli_e = mnli_tokenizer(
                    mnli_input, return_tensors="pt", truncation=True
                )
                mnli_pred = torch.softmax(mnli_model(**mnli_e).logits, dim=1).tolist()[
                    0
                ]
